...
def play_game():
    ...
    def gameLoop():
        ...
        while not game_over:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    game_over = True
                        ...
                        if event.key == pygame.K_UP and green_first_move:
                            x_green_change, y_green_change = 0, -block_size
                            green_first_move = False
                        elif event.key == pygame.K_DOWN and green_first_move:
                            x_green_change, y_green_change = 0, block_size
                            green_first_move = False
                        elif event.key == pygame.K_LEFT and green_first_move:
                            x_green_change, y_green_change = -block_size, 0
                            green_first_move = False
                        elif event.key == pygame.K_RIGHT and green_first_move:
                            x_green_change, y_green_change = block_size, 0
                            green_first_move = False

                        # Pierwszy ruch niebieskiego
                        elif event.key == pygame.K_w and blue_first_move:
                            x_blue_change, y_blue_change = 0, -block_size
                            blue_first_move = False
                        elif event.key == pygame.K_s and blue_first_move:
                            x_blue_change, y_blue_change = 0, block_size
                            blue_first_move = False
                        elif event.key == pygame.K_a and blue_first_move:
                            x_blue_change, y_blue_change = -block_size, 0
                            blue_first_move = False
                        elif event.key == pygame.K_d and blue_first_move:
                            blue_first_move = False
                            x_blue_change, y_blue_change = block_size, 0

                        # Nie pierwszy ruch zielonego
                        if event.key == pygame.K_UP and not green_first_move and (x_green_change != 0 and y_green_change != -block_size):
                            x_green_change, y_green_change = 0, -block_size
                        elif event.key == pygame.K_DOWN and not green_first_move and (x_green_change != 0 and y_green_change != block_size):
                            x_green_change, y_green_change = 0, block_size
                        elif event.key == pygame.K_LEFT and not green_first_move and (x_green_change != -block_size and y_green_change != 0):
                            x_green_change, y_green_change = -block_size, 0
                        elif event.key == pygame.K_RIGHT and not green_first_move and (x_green_change != -block_size and y_green_change != 0):
                            x_green_change, y_green_change = block_size, 0
            ...
            if len(green_snake_list) > length_of_green_snake:
                del green_snake_list[0]

            if len(blue_snake_list) > length_of_blue_snake:
